...
net_final = tf.keras.models.load_model('weights-improvement-02-0.51.h5',custom_objects={'binary_crossentropy_plus_dice_loss': bce_dice_loss,'iou_score': iou_score})


# In[6]:


val_generator = DataGenerator(dir='val_100', img_col='sat', mask_col='mask', shuffle=False, batch_size=1)
val_generator.df
...
predict_test=net_final.predict_generator(val_generator, verbose=1)
predict_test.size
...
predict_test = np.reshape(predict_test, (100, 1024, 1024))
predict_test
...
val_generator = DataGenerator(dir='val_100', img_col='sat', mask_col='mask', shuffle=False, batch_size=100)
val_generator.df
...
fpr, tpr, thersholds = metrics.roc_curve(y_test, predict_test, pos_label=1,drop_intermediate=0)
...
plt.xlim([-0.1, 1.1])
plt.ylim([-0.1, 1.1])
plt.xlabel('False Positive Rate') #横座標是fpr
...
for i in range(0,len(fpr),20):

    columnA = (thersholds[i])
...
#     predict = convert_binary(predict,0.00721795344725251)
...
#     return final_conv
# predict_test = convert_binary(predict_test, 0.5)
# predict_test
...
# plt.imshow((np.reshape(predict_test*100, (1024, 1024))))
...
# import cv2
# cv2.imshow('My Image', np.reshape(predict_test*100000, (1024, 1024)))
...
# cv2.imwrite('output.png',  np.reshape(predict_test, (1024, 1024))*255)
